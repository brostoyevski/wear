...
public class CubeView : UIView {
	...
	public override void ViewWillAppear (UIView sourceView = null) {
		...
		print("do will appear on cubeview");
		run = true;
	...
	void FixedUpdate ()
	{
		if (run) {
			...
			if (System.DateTime.UtcNow.Minute < 10) {
				m = "0" + System.DateTime.UtcNow.Minute;
			} else {
				m = System.DateTime.UtcNow.Minute.ToString ();
			}
			if (System.DateTime.UtcNow.Second < 10) {
				s = "0" + System.DateTime.UtcNow.Second;
			} else {
				s = System.DateTime.UtcNow.Second.ToString ();
			}
			...
			if (System.DateTime.UtcNow.Second % 2 == 0) {
				...
				cuber [last_cuber].transform.position = new Vector3 (col - 3, row, 11.5f);
